...
export const BudgetTierCard = ({ budgetProgress = [], onAddBudget }) => {
  ...
  // Fallback mock data when no backend budgets loaded
  const mockBudgets = [
    { id: 'b1', category_name: 'Housing', allocated_amount: 2400, spent_amount: 1800, color: '#4382DF', status: 'on_track' },
    { id: 'b2', category_name: 'Groceries', allocated_amount: 1200, spent_amount: 660, color: '#10B981', status: 'safe' },
    { id: 'b3', category_name: 'Entertainment', allocated_amount: 800, spent_amount: 720, color: '#F59E0B', status: 'near_limit' },
  ];

  const displayBudgets = budgetProgress.length > 0 ? budgetProgress.slice(0, 3) : mockBudgets;
      ...
      <div>
        ...
        <div className="space-y-4">
          {displayBudgets.map((b, idx) => {
            const allocated = b.allocated_amount || b.allocated || 1000;
